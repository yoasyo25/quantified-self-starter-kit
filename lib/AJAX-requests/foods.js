var $ = require('jquery'),
    foodHandlers = require('../response-handlers/foods'),
    foodUrl = 'http://quantified-self-api-aa-ya.herokuapp.com/api/v1/foods',
    mealsUrl = 'http://quantified-self-api-aa-ya.herokuapp.com/api/v1/meals';
...
var postFood = function (foodPost) {
  $.ajax({
    ...
    success: function (data) {
      $('.food-table').prepend(`<tr data-id="${data.id}"><td contenteditable="true">${data[0].name}</td><td contenteditable="true">${data[0].calories}</td><td class="delete-cell"><i class="delete-button fa fa-minus-circle" aria-hidden="true"></i></td></tr>`)
